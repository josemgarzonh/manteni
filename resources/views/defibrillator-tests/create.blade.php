    ...
    <div class="pd-20">
        <form id="testForm" action="{{ route('defibrillator-tests.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ $value }}</label>
                <div>
                    <input type="radio" name="{{ $key }}" value="1" required class="mr-2"> Cumple
                    <input type="radio" name="{{ $key }}" value="0" class="ml-4 mr-2"> No Cumple
                </div>
            </div>
            @endforeach
...
{{-- ========================================================== --}}
{{-- CAMBIO IMPORTANTE: ENVOLVEMOS EL SCRIPT EN @push('scripts') --}}
{{-- ========================================================== --}}
@push('scripts')
